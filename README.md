...
`k` opens the kit inventory: which kits exist, the monitor each claims, whether it is complete (card + query + runner), and where the local copy stands against the committed one. `v` toggles between the base-rate card and the query, `d` shows local changes, `o` opens the folder.

```powershell
oce-sentry --kits
```

This is the collaboration surface. Kits are generated by the ODSP SRE tooling and consumed here, so "what changed upstream" is an operational question: a kit whose query was verified against a schema months ago is a different artifact from one verified last week. Nothing on this screen edits or fetches a kit — they belong to the team that generates them.
